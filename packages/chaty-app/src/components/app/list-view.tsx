import React, { ReactNode, useState } from 'react'
import { ulid } from 'ulid'

interface Props {
  /**
   * Space to leave at the top of the scroll container
   */
  offsetTop: number

  /**
   * Handle when the top of the list is reached
   * @param reposition Scroll guard callback
   */
  fetchTop(reposition: (cb: () => void) => void): Promise<void>

  /**
   * Handle when the bottom of the list is reached
   * @param reposition Scroll guard callback
   */
  fetchBottom(reposition: (cb: () => void) => void): Promise<void>

  /**
   * Render the list itself
   */
  children: ReactNode
}

/**
 * Dynamic list view with ability to move through history
 *
 * This component only provides the scrolling behaviour
 */
export function ListView({ fetchTop, fetchBottom, children, offsetTop }: Props) {
  const [fetching, setFetching] = useState(false)
  const id = ulid()

  function onScroll(event: React.UIEvent<HTMLDivElement>) {
    if (fetching) return

    // Calculate our current position in the scroll container
    const scrollHeight = event.currentTarget.scrollHeight
    const relativeY = event.currentTarget.scrollTop - event.currentTarget.getBoundingClientRect().height
    const absoluteY = scrollHeight + relativeY

    /**
     * Guard scroll position when rendering to DOM
     * @param cb Render callback
     * @param multiplier Direction multiplier
     */
    function scrollGuard(cb: () => void, multiplier = 1) {
      // Get initial state of scroll container
      const element = document.getElementById(id)
      const previousHeight = element?.scrollHeight ?? 0
      const previousTop = element?.scrollTop ?? 0

      // Render the new DOM nodes
      cb()

      // Example :
      // Before inserting new content
      // const previousHeight = element.scrollHeight // 2000px
      // const previousTop = element.scrollTop // 500px
      //
      // // Insert new content at the top
      // cb()
      //
      // After: scrollHeight = 2500px (new content added)
      // After: scrollTop still = 500 (but now you're seeing wrong content!)
      //
      // Calculate the visual shift
      // const shift = previousHeight - element.scrollHeight + previousTop - element.scrollTop
      // shift = 2000 - 2500 + 500 - 500 = -500
      // element.scrollBy({ top: multiplier * -500 })

      // Offset the difference
      element?.scrollBy({
        top: multiplier * (previousHeight - element.scrollHeight + previousTop - element.scrollTop),
      })
    }

    if (absoluteY <= offsetTop) {
      // Fetch upwards if we scroll to the top
      setFetching(true)
      fetchTop((cb) => scrollGuard(cb)).finally(() => setFetching(false))
    } else if (event.currentTarget.scrollTop === 0) {
      // Fetch downwards if we scroll to the bottom
      setFetching(true)
      fetchBottom((cb) => scrollGuard(cb, -1)).finally(() => setFetching(false))
    }
  }

  return (
    <div id={id} className='h-full flex flex-col-reverse' style={{ flexGrow: 1 }} onScroll={onScroll}>
      <div className='flex flex-col-reverse'>{children}</div>
    </div>
  )
}
