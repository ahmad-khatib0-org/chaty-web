...
import { RE_UNICODE_EMOJI, unicodeEmojiUrl } from '@/components/markdown/emoji'
import { isInCodeBlock } from './code-mirror-common'
import { parseUnicodeEmoji } from '@/components/markdown/plugins/unicode-emoji'
import { Channel, ServerMember, ServerRole, User } from 'chaty-client/models'
  ...
  const widgetMatcher = new MatchDecorator({
    ...
    decoration: ([str, unicodeEmoji, emojiId, userId, channelId, roleId], view, pos) => {
      ...
      if (unicodeEmoji) {
        ...
        widget = new EmojiWidget(`${client?.configuration?.features?.files?.url}/emojis/${emojiId}`)
      } else if (userId) {
        const member = serverId
          ? getClient.serverMembers.getByKey({ server: serverId, user: userId })
          : undefined
