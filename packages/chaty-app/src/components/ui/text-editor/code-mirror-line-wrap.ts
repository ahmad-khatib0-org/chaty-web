...
function measureTextWidth(text: string, font: string): number {
  ...
  const canvas: HTMLCanvasElement =
    ...
    ((measureTextWidth as unknown as { canvas: HTMLCanvasElement }).canvas = document.createElement('canvas'))
  ...
  if (mainFont !== prev?.mainFont) {
    ...
    if (monoFont !== prev?.monoFont) {
      ...
      viewupdate.view.dispatch({
        effects: [CharacterWidthEffect.of({ mainFont, monoFont, width })],
      })
    }
  }
})
...
const lineWrapDecorations = StateField.define({
  ...
  update(deco, tr) {
    ...
    for (let i = 0; i < tr.state.doc.lines; i++) {
      ...
      const matched_chars = /^(\s*)(?:(?:(?:[-*+]\s|\d+\.\s)\s*(?:\[[\sxX]\]\s+)?|>\s+))*/
      ...
      decorations.push(whitespaceDecoration.range(line.from, line.from + groups[0].length))
    }
