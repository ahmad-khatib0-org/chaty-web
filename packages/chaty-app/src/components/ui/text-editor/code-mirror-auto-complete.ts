...
import { autocompletion, closeCompletion, currentCompletions } from '@codemirror/autocomplete'

import { AutoCompleteSearchSpace } from '@/components/common'
import { codeMirrorAutoCompleteSource } from './code-mirror-auto-complete-source'
...
export function codeMirrorAutoComplete(searchSpace?: AutoCompleteSearchSpace) {
  const extension = autocompletion({
    ...
    tooltipClass: (state) => {
      ...
      } else if (completions[0]?.type == 'user' || completions[0]?.type == 'role') {
