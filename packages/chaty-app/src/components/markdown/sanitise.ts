 ...
 * Matches lines with 4+ levels of nested blockquotes (>) or lists (*, +, -)
 ...
 * Matches one or more lines that start with > (optional leading spaces)
 * Example: "> quote line 1\n> quote line 2"
 */
const RE_BLOCKQUOTE = /^([^\S\r\n]*>[^\n]+\n?)+/gm

/**
 * Regex for matching HTML tags
 * Matches HTML open/close tags at start of line, captures tag and remaining text
 * Example: "<div>rest of line" or "</p>text"
 */
const RE_HTML_TAGS = /^(<\/?[a-zA-Z0-9]+>)(.*$)/gm

/**
 * Regex for matching empty lines
 * Matches lines containing only whitespace or nothing at all
 * Example: "   " or "" (blank line)
 */
const RE_EMPTY_LINE = /^\s*?$/gm

/**
 * Regex for matching line starting with plus
 * Matches lines starting with + (may have leading spaces), but not ++ or +text
 * Example: "+ " or "+" but not "+item" or "++"
 */
const RE_PLUS = /^\s*\+(?:$|[^+])/gm

/**
 * Regex for matching non-breaking spaces in code blocks
 * Replaces \uF800 (private use character) with empty line in code blocks
 * Used to fix empty lines inside triple backticks (```)
 * Matches pattern: \n\uF800\n between code block boundaries
 */
...
export function remarkInsertBreaks() {
  return (tree: import('hast').Root) => {
     ...
     * @returns Element
     */
    function recurse(element: Record<string, unknown> & { children: unknown[] }): unknown {
