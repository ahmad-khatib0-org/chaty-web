...
import { RE_ANY_EMOJI, CustomEmoji, UnicodeEmoji } from '.'

/**
 * Convert any arbitrary text to rich text with emoji
 */
export function TextWithEmoji({ content }: { content: string }) {
  /**
  ...
  function render(content?: string) {
    ...
    while (match) {
      ...
      components.push(
        match[0].length === 28 ? <CustomEmoji id={match[1]} /> : <UnicodeEmoji emoji={match[0]} />
      )
