        ...
        if (child.type === 'text') {
          ...
          if (searchingForEnd !== -1) {
            ...
            node.children.splice(i, 0, {
              type: 'spoiler',
              children: [...spoilerContent, ...elements, { type: 'text', value: components.shift() }],
            })
          ...
          if (innerElements) {
            for (let j = 0; j < innerElements; j++) {
              node.children.splice(
                i + 1,
                0,
                { type: 'spoiler', children: [{ type: 'text', value: components.shift() }] },
                { type: 'text', value: components.shift()! }
              )
          ...
          if (spillOver) {
            searchingForEnd = i + 1
            spoilerContent.push({ type: 'text', value: components.pop() })
          }
        }
      }
    }
